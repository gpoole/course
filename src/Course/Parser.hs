...
satisfy ::
  (Char -> Bool)
  -> Parser Char
-- satisfy cons = character >>= \c ->
--   bool
--     (unexpectedCharParser c)
--     (pure c)
--     (cons c)
satisfy cons = character >>=
  lift3 bool
    unexpectedCharParser
    pure
    cons
...
firstNameParser =
  ((:.) <$> upper) <*> list lower
  -- The first Result is a function that will concatenate a string after the parsed letter
  -- this function is applied to the result of the second parser
  -- a                  b
  -- parsing Abc
  -- Parser (:.) A
  --
...
instance Functor Parser where
  ...
  (<$>) f (P p) = P (\i -> case p i of
    ErrorResult e -> ErrorResult e
    Result j a -> Result j (f a)) -- apply f to the result of the parser
               -- Result j ((:.) A)
...
instance Applicative Parser where
  ...
  pure = valueParser
  (<*>) ::
    Parser (a -> b) -- a partially applied result; the result is a function a -> b
    -> Parser a -- parse
    -> Parser b -- parse
  (<*>) f a =
    f >>= \g -> -- result (a -> b)
...
instance Monad Parser where
  ...
  (=<<) f (P p) =
    P (\i -> case p i of
      ErrorResult e -> ErrorResult e
      Result j a -> parse (f a) j)
-- Take a parser, insert a new parser taking the output of the previous parser
-- \i -> newParser i =<< oldParser
-- newParser =<< oldParser
